...
public class ConditionService {
    ...
    public ConditionResponseDto analyzeCondition(ConditionRequestDto request) {
        ...
        try {
            String feedbackJson = new ObjectMapper().writeValueAsString(result.getConditionItems());
            condition.updateAnalysis(result.getConditionTitle(), feedbackJson);
    ...
    private String buildSystemPrompt() {
        return """
            ...
            {
              "conditionTitle": "한 줄 컨디션 요약 (예: 최고의 컨디션이에요!)",
              "conditionItems": [
                "수면 관련 한 줄 문장",
                "최근 운동 현황 관련 한 줄 문장",
                "통증 부위 관련 한 줄 문장"
